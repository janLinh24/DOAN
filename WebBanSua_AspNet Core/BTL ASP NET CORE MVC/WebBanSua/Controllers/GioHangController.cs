    ...
    public class GioHangController : Controller
    {
        ...
        public IActionResult UpdateCart(int maSP, int soLuong)
        {
            ...
            try
            {
                if (gioHang != null)
                {
                    CartItem item = gioHang.FirstOrDefault(p => p.sanPham.MaSp == maSP);
                    if (item != null)
                    {
                        item.soLuong = soLuong;
                    }
                    // Cập nhật giỏ hàng trong session sau khi cập nhật số lượng sản phẩm
                    HttpContext.Session.Set<List<CartItem>>("GioHang", gioHang);
                }
                else
                {
                ...
                // Trả về số lượng tổng các sản phẩm trong giỏ hàng để cập nhật giao diện người dùng
                return Json(new
                {
